...
class Plugin {
  ...
  async getNameByHash(nameHash, height) {
    const db = this.node.chain.db;
    const next = height + this.network.names.treeInterval;
  ...
  initChainRoutes() {
    ...
    this.nodeHTTP.get('/nameinfo/:height/hash/:hash', async (req, res) => {
      ...
      const checkTip = valid.int('tip', height);

      if (!hash || !height)
        throw new Error('Needs hash and height');

      const ns = await this.getNameByHash(hash, height);
      ...
      return res.json(200, ns.getJSON(checkTip, this.network.type));
    });
